...
public class FileConfig extends Config {
    ...
    private void xmlLoad(Path configPath) throws InvalidConfigException {
        ...
        for (Node domNode = domRoot.getFirstChild(); domNode != null; domNode =
                domNode.getNextSibling()) {
            if (domNode.getNodeType() == Node.ELEMENT_NODE) {
                Element domElement = (Element) domNode;
                switch (domElement.getNodeName()) {
                    case "leveldb":
                        domLeveldb = domElement;
                        break;
                    case "neo4j":
                        domNeo4j = domElement;
                        break;
                    case "details":
                        domDetails = domElement;
                        break;
                    case "entities":
                        domEntities = domElement;
                        break;
    ...
    private void xmlLoadDetails(Element domDetails) {
        for (Node domNode = domDetails.getFirstChild(); domNode != null; domNode =
                domNode.getNextSibling()) {
            if (domNode.getNodeType() == Node.ELEMENT_NODE) {
